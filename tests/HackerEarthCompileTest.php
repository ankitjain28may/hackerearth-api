...
class HackerEarthCompileTest extends TestCase
{
    ...
    public function testInvalidAPIKey()
    {
        ...
        $result = $this->hackerearth->Compile([$data]);
        $this->assertEquals($result, '["Client error: `POST https:\/\/api.hackerearth.com\/v3\/code\/compile\/` resulted in a `403 Forbidden` response:\n{\"message\": \"UnregisteredClientError: Client does not exist\", \"errors\": {}}\n"]');
    }


    public function testWithEmptyInput()
    {
    	$result = $this->hackerearth->Compile();
